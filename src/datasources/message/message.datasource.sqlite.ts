...
export class MessageDatasourceSQLite implements MessageDatasource {
  ...
  constructor() {
    this.findByUserIdStatement = dbInstance.prepare(`SELECT * FROM ${GLOBAL_CONFIG.database.tableMessage} WHERE userId = :userId`);
    this.addMessageOfUserStatement = dbInstance.prepare(`INSERT INTO ${GLOBAL_CONFIG.database.tableMessage} (userId, content) VALUES (:userId, :content) RETURNING *`);
  }
  ...
  addMessageOfUser(userId: number, content: string): Promise<Result<Messages, Error>> {
    return Result.try(
      () => {
        ...
        if(!isMessage(message)) throw new Error(`The user with id: ${userId} not exist, isn't possible save that message`);
        return message;
      }
    )
  }
